...
/**
 * A interface representing methods used when a class implements Permissions. This should usually be used when a class has Permissions that restrict what players can do.
 * 
 * For example a Command would implement Permissions to ensure that only certain players can use certain commands or options within it. A plugin would also implement permissions to provide a shortcut for Commands to register new permissions and to create wildcarded shortcuts for example.
 */
