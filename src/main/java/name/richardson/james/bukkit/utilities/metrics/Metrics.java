/*******************************************************************************
 ...
 Metrics.java is part of BukkitUtilities.
...
public class Metrics {
	...
	public File getConfigFile() {
		// I believe the easiest way to get the base folder (e.g craftbukkit set via
	 ...
	 * task as the plugin and send the initial data to the metrics backend, and
	 * then after that it will post in increments of PING_INTERVAL * 1200 ticks.
	 * 
	...
	public boolean start() {
		synchronized (this.optOutLock) {
			...
			this.task = this.plugin.getServer().getScheduler().runTaskTimerAsynchronously(this.plugin, new Runnable() {
				...
				public void run() {
					try {
						// This has to be synchronized or it can collide with the disable
						// method.
						...
						// We use the inverse of firstPost because if it is the first time
						// we are posting,
	...
	private void postPlugin(final boolean isPing) throws IOException {
		...
		// END server software specific section -- all code below does not use any
		...
		// Acquire a lock on the graphs, which lets us make the assumption we also
		// lock everything
		...
		synchronized (this.graphs) {
			...
			while (iter.hasNext()) {
				...
				for (final Plotter plotter : graph.getPlotters()) {
					...
					// The format is C-GRAPHNAME-PLOTTERNAME where separator - is defined
					// at the top
					...
					// The value to send, which for the foreseeable future is just the
					// string
